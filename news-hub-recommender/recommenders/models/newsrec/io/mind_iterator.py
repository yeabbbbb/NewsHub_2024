...
class MINDIterator(BaseIterator):
    ...
    def load_news_from_db(self, candidate_news):
        self.init_news_from_db(candidate_news)

        candidate_title_index = np.asarray(
            self.news_title_index, dtype=np.int32
        )

        if hasattr(self, "news_title_index"):
            delattr(self, "news_title_index")

        return candidate_title_index
    
    def load_user_from_db(self, user_id, clicked_news):
        self.init_news_from_db(clicked_news)

        user_indexes = [self.uid2index.get(user_id, 0)]

        if len(self.news_title_index) < 50:
            padding = np.zeros((self.his_size - len(self.news_title_index), 30))
            self.news_title_index = np.vstack([self.news_title_index, padding])

        click_title_indexes = []
        click_title_indexes.append(self.news_title_index)

        user_indexes = np.asarray(user_indexes, dtype=np.int32)
        click_title_indexes = np.asarray(click_title_indexes, dtype=np.int64)

        if hasattr(self, "news_title_index"):
            delattr(self, "news_title_index")

        return [click_title_indexes, user_indexes]

    def init_news_from_db(self, news_source):
        self.nid2index = {}
        news_title = []

        for row in news_source:
            nid, title = row
