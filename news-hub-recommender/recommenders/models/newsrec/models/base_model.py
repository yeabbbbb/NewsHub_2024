...
class BaseModel:
    ...
    def predict(self, user_id, candidate_news, clicked_news):
        news_vecs = self.run_news_on_db(candidate_news)
        user_vec = self.run_user_on_db(user_id, clicked_news)[0]

        return np.dot(news_vecs, user_vec)
        
    def run_news_on_db(self, candidate_news):
        if not hasattr(self, "newsencoder"):
            raise ValueError("model must have attribute newsencoder")

        news_input = self.test_iterator.load_news_from_db(candidate_news)
        news_vecs = self.newsencoder.predict(news_input)

        return news_vecs
    
    def run_user_on_db(self, user_id, clicked_news):
        if not hasattr(self, "userencoder"):
            raise ValueError("model must have attribute userencoder")
        
        user_input = self.test_iterator.load_user_from_db(user_id, clicked_news)
        user_vec = self.userencoder.predict(user_input)
        return user_vec